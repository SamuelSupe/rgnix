...
相对 root、include、证书和脚本路径均基于主配置文件目录。挂载示例目录只读时无需其他数据卷；日志写 stdout/stderr。使用文件日志时给运行用户写权限。错误日志在加载配置时打开；访问日志由异步线程按需打开，写入失败会记录错误。未实现 NGINX USR1 日志重开协议，建议交给容器日志系统轮转。
...
```sh
docker build --secret id=build_ca,src=/path/to/ca-bundle.pem -t rgnix:0.1.0 .
```
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -t YOUR_REGISTRY/rgnix:0.1.0 \
  --output type=oci,dest=rgnix-0.1.0.oci.tar .
```
...
- 不支持 ExternalName、resource backend、UDP/SCTP。无端点返回 503；Ingress 后端使用 HTTP，HTTPS 上游目前通过独立配置模式提供。
- 冲突的 host/path/pathType 按创建时间、namespace、name 升序，较早者获胜；发布 RouteConflict Event。通配域名仅匹配一层 DNS 标签。
- Ingress 转发 Host 为客户端主机，X-Forwarded-For/X-Real-IP 为直接连接的客户端 IP，X-Forwarded-Proto 为当前协议；不会盲目信任传入转发头。不提供外部 LB 真实 IP 信任链或 PROXY protocol 配置。
...
每个使用插件的 Ingress 对应一个 `rgnix-state-*` ConfigMap，位于 publish-service 的命名空间。检查点包含版本化源码和路由定义，绑定 Ingress UID、插件 ConfigMap UID 和 IngressClass；不保存 TLS 私钥或 Wasmtime 原生机器码。写入使用 resourceVersion 并核对当前源资源，Ingress 删除或插件撤销后清理。源码与路由必须能够编码在 900 KiB 检查点内。持久化异步执行，但插件和路由只有在 watch 确认该检查点后才能发布；写入失败继续使用上一有效版本，增加指标并重试。首次发布等待期间保持 NotReady。部署更新前应确认 `rgnix_checkpoint_healthy=1` 且没有 PendingCheckpoint 诊断。
...
两个副本都维护数据面，Lease **只用于状态回写**。Lease 位于 publish-service 的命名空间，名为 `CLASS-leader`，30 秒租期，约 10 秒续租；身份默认 POD_NAME。领导者把 `--publish-service namespace/name` 的 `status.loadBalancer` 地址复制到所选 Ingress，撤销地址时清空旧值。状态更新使用 resourceVersion，资源删除和冲突不覆盖新版本。
...
RBAC 包含全命名空间资源 watch、Secret 读取、Ingress status patch、Event create，以及 publish namespace 内 Lease 操作。IngressClass 的筛选不缩小 Kubernetes RBAC 权限；按集群管理组件保护 ServiceAccount 和可写插件 ConfigMap。

更新 Chart 时同时更新 RBAC：publish namespace 的 Role 还需要 ConfigMap get/list/create/update/delete 权限，用于检查点恢复和清理。
...
健康状态按端点统计；正常完成的响应清除连续失败计数，客户端取消不计为上游失败，HTTP 5xx 本身不触发摘除。相同后端配置在快照更新间保留健康状态。所有端点不可选时返回 503，失败的当前请求不会自动重试。这是被动故障隔离，不会主动请求应用的健康 URL；Ingress 还使用 EndpointSlice 的就绪状态。该默认策略与 NGINX 默认 max_fails 参数不同。

并发上限应与容器内存、请求体和插件复杂度一起调整；它不限制尚未完成请求头的空闲 TCP 连接总数，也不是生产容量保证。

## 管理与指标

独立模式默认只在 `127.0.0.1:9090` 开放管理接口；Ingress Pod 内监听 `0.0.0.0:9090`，业务 Service 不暴露。管理接口未提供认证，使用网络策略/端口隔离控制访问。
...
| `rgnix_access_logs_dropped_total` | 访问日志队列溢出 |
...
指标不使用请求路径、host、租户或其他任意请求值作为标签。进程 CPU/RSS 使用容器监控或 `/proc` 获取。
