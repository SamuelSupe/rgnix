...
The v0.1.0 preview archive contains the qa14 binary from the verified Linux arm64 image. It is an AArch64 ELF executable built on Debian 12, not a macOS or Alpine/musl binary.
...
The exact binary SHA-256 and validation environment are in [the artifact record](validation/semantics-artifact.json). Runtime compatibility on other distributions is not separately certified.

## Verify and run

Download `rgnix-0.1.0-linux-arm64.tar.gz` and `SHA256SUMS` from the same release, then run:

```sh
sha256sum -c SHA256SUMS
tar -xzf rgnix-0.1.0-linux-arm64.tar.gz
cd rgnix-0.1.0-linux-arm64
