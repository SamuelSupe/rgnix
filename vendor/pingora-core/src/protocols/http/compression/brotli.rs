...
impl Encode for Compressor {
    fn encode(&mut self, input: &[u8], end: bool) -> Result<Bytes> {
        ...
        // flush only ends a metablock. into_inner emits the stream termination
        // bits; dropping the writer after sending its Vec loses those bytes.
        let output = if end {
            self.compress.take().unwrap().into_inner()
        } else {
            std::mem::take(compress.get_mut())
        };
        self.total_out += output.len();
        self.duration += start.elapsed();
        Ok(output.into())
    }
...
mod tests_stream {
    ...
    fn compress_brotli_data() {
        ...
        brotli::BrotliDecompress(&mut std::io::Cursor::new(compressed), &mut decoded).unwrap();
        assert_eq!(decoded, b"adcdefgabcdefgh\n");
    }
}
