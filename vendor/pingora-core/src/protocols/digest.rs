...
pub struct SocketDigest {
    ...
    /// Client address asserted by a trusted PROXY protocol sender. The kernel
    /// peer_addr remains available for trust decisions and audit logging.
    pub proxy_protocol_addr: OnceCell<Option<std::net::SocketAddr>>,
}
...
impl SocketDigest {
    ...
    pub fn from_raw_fd(raw_fd: std::os::unix::io::RawFd) -> SocketDigest {
        SocketDigest {
            ...
            proxy_protocol_addr: OnceCell::new(),
        }
    }
