...
pub trait ProxyHttp {
    ...
    /// Per-attempt budget including connection setup and the complete response stream.
    /// The transaction deadline, when present, remains the upper bound.
    fn backend_request_timeout(&self, _session: &Session, _ctx: &Self::CTX) -> Option<Duration> {
        None
    }
